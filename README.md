...
> **Current profile: `Concept only / No execution evidence / Maintainer-reviewed / Public documentation / Documentation-only / No external assurance claim`.** Reversible solo sandbox work is **Exploration permitted**. Any sandbox artifact has the stricter profile `Experimental / Synthetic evidence / Maintainer-reviewed / Synthetic data / Local-only / No external assurance claim`; it is not a supported prototype, validated capability, MVP component, or deployment.
...
During the Current solo-concept state, useful contributions include documentation work and compliant disposable experiments permitted by the [Current work authorization](docs/current-work-authorization.md). Their activity status is **Exploration permitted**, not validated or approved. Do not describe sandbox behavior as implemented product capability, use it as external evidence, or promote it into supported prototype, MVP, pilot, or production work without the destination evidence required by the delivery plan. `Blocked` applies only to a named failed promotion or deployment decision, never to the repository or unrelated exploration.
