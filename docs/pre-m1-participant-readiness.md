...
Before contact, the maintainer must identify the target role; document and approve the privacy notice, applicable lawful basis, storage and contact routes, access controls, retention/deletion rules, withdrawal route, and disclosure-review controls for the minimum contact/role/interest/conflict fields; and preserve the relying-party independence rule and all role disqualifiers. The storage route must be outside Git, approved for this purpose, and limited to those fields. The candidate must receive the repository, concise description, boundaries, option to decline, and privacy notice when information is requested or received. The same controls apply to the initial fields, any acknowledgement, and any later permitted non-public information.
