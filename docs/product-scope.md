...
The discovery hypothesis is that some providers may export a flight log or prepare a signed compliance report through a contract document channel. Discovery must establish whether either artifact exists in the selected workflow, who requests or reviews it, what decision (if any) it informs, and its coverage unit and frequency. It must not assume that a site officer checks speed, creates an exception, or makes one decision per deliverable. A candidate product intervention would replace only an evidenced disclosure-heavy speed-evidence step by producing and independently verifying a claim that an eligible telemetry snapshot's normalized horizontal speed was at or below the public policy maximum. A snapshot does not summarize an entire flight, the proof does not establish that the sensor was truthful, and neither proof generation nor verification authorizes flight or payment.

The candidate restricted-field hypothesis is that providers may need to withhold exact coordinates, complete trajectories, fine-grained timestamps, vehicle or customer identifiers, mission objectives, imagery, or unrelated telemetry because those fields may expose site layout, customer identity, operating methods, or route planning. Provider and relying-party discovery must test that field-by-field candidate, the asserted harms, and the minimum public policy, proof-version, freshness, validity, and replay metadata needed for the decision. No field is treated as restricted or necessary merely because it appears in this candidate list.

Decision timing, decision unit, review sequence, and volume are unresolved. The working frequency hypothesis is **5–50 decisions per buyer per month**, but discovery must replace it with observed buyer data and must test rather than presume a per-deliverable decision. Candidate consequences to test are that false acceptance may permit contractually non-compliant evidence to pass review or prompt investigation or reinspection, while false rejection may delay review or payment, add analyst work, prompt a repeat inspection, or damage trust. These are not observed consequences or severity findings, and proof acceptance is not a safety guarantee.

The comparison hypothesis is that a zero-knowledge claim may provide a useful disclosure or independent-verification advantage over the workflow's actual signed-report and contractual-control alternatives. Discovery must compare the artifacts, required trust, requested underlying data, copying and retention, access and onward-use controls, review burden, and decision fitness; it may conclude that a minimized signed report or another existing control is sufficient and that a zero-knowledge proof adds no material value. Signatures, contracts, authentication, and audit controls remain candidate surrounding controls rather than substitutes presumed inadequate.
...
- Obtain an explicit product-owner determination that the approved evidence is sufficient or insufficient to validate the problem and workflow for this gate; the determination must cite the sampling rationale, paired coverage, buyer/procurement evidence, contradictions, and limitations and must not treat recruitment targets as validation evidence.
...
- Evidence that no restricted field appears in the Phase 3 offline package, verifier output, logs, mock publication record, or any justified operator view/API during the paired integrations.
