...
Lifecycle tests MUST exercise every permitted transition and reject skipped, reversed, unsigned, unauthorized, wrong-digest, out-of-order, and in-place mutation attempts. With an authenticated boundary clock, cases MUST cover just before, exactly at, and just after `not_before` and `not_after`; activation before approval or before `not_before`; deprecated acceptance before sunset with a warning; and rejection after expiry. Matrix cases MUST pair each policy with supported and unsupported schema, statement, circuit, proof-suite, verification-key, tier, and domain identifiers, proving that cryptographic validity cannot override incompatibility.
