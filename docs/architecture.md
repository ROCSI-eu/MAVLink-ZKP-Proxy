...
Lifecycle and interfaces follow the delivery-plan milestones rather than assuming a hosted service:

- **Phase 0 — decision framing and scaffold:** versioned schemas and deterministic fixtures define file/stdin boundaries. Non-interactive tooling emits stable machine-readable validation errors.
- **Phase 1 — telemetry vertical slice:** offline record/replay produces canonical records; no network API, tenant model, or hosted lifecycle is required.
- **Phase 2 — proof spike and local verification:** a self-contained package and experimental verifier CLI or minimal library verify fixtures from files/stdin without a witness, vendor service, or network dependency. Typed results keep cryptographic, policy, freshness, revocation, replay, assurance, and optional publication dimensions separate.
- **Phase 3 — mock adapter and operator boundary (MVP cutoff):** the offline package remains authoritative and independently verifiable. Validated workflow evidence MAY justify the smallest authenticated single-organization or explicitly non-multi-tenant submit/status API and minimal operator view. Multi-tenant administration, cross-tenant exports, managed webhooks, per-tenant quotas, billing identifiers, and organization/environment hierarchy are post-MVP control-plane concerns.

When Phase 3 persistence or an API is justified, the normative lifecycle states are:
...
Transitions are monotonic; retries add attempt metadata rather than moving finalized state backward. `VERIFIED` means a typed verifier result was recorded, not that every dimension passed. Publication remains separate: `SUBMITTED` and `FINALIZED` describe only the optional adapter outcome and never rewrite `envelope_evaluation`. Any `service_disposition` is derived convenience data, and the relying party owns its separate business decision. Stable machine-readable errors and lifecycle outputs MUST exclude restricted data.

The file/stdin package and verifier are the complete required interface through Phase 2 and remain required in Phase 3. They use explicit schema, claim, circuit, policy, encoding/domain, and proof-system versions; missing, unknown, revoked, or incompatible security-relevant versions fail closed. Verification is non-interactive and offline after authenticated artifacts and status/revocation snapshots are imported.

If workflow evidence justifies a Phase 3 API, its behavioral surface is limited to submitting one canonical package and reading its redacted result/lifecycle. `SubmitClaim` rejects raw telemetry and witness fields and requires an opaque idempotency key; identical retries return the original outcome, changed content returns stable `IDEMPOTENCY_CONFLICT`, concurrent duplicates converge, and timeouts are reconciled with the same key or a status read. `GetClaimStatus` returns only typed verifier dimensions, safe reason codes, expiry, versions, and optional publication state. A separately authorized package read MAY return the portable proof, reviewed public inputs, and authenticated verification material needed for independent verification.

Every implemented boundary is allowlisted. Files, stdin/stdout, URLs, logs, traces, metrics, errors, lifecycle records, API payloads, operator views, and mock records exclude raw telemetry, witnesses, exact speed/position, unapproved precise times, stable source identity, salts/openings, secrets, customer details, and mission details. Contract technology remains **Open**, and the Phase 3 API remains experimental until executable tests cover its implemented authorization rules, canonical serialization, idempotency where mutations exist, stable error families, version combinations, lifecycle, and restricted-field exclusion. Post-MVP control-plane features are not part of that stability gate.
