...
The Current state is solo concept work with `M1` `in_progress` and `M2` blocked. In addition to reconciliation and bounded outreach, the maintainer may run the delivery plan's disposable sandbox experiments using demonstrably synthetic data. All artifacts and outputs carry **`EXPERIMENTAL`**, **`SYNTHETIC_ONLY`**, and **`NOT VALIDATION OR PRODUCTION AUTHORIZATION`** and are isolated from real telemetry, live ledgers, hardware/commands, credentials, participant data, and production infrastructure. They are not discovery or gate evidence. Independent and external evidence is required for the applicable promotion or external claim, not for sandbox creation.
...
| --- | --- | --- |
| [Current work authorization](current-work-authorization.md) | Work permitted during solo-concept M1 and the transition boundary into later work | Repository maintainer for factual maintenance; later-work roles remain risk-triggered |
