...
| Topic | State | Due gate | Accountable role | Required evidence |
| --- | --- | --- | --- | --- |
| Observational, single-vehicle SITL MVP with no command path | Proposed scope baseline | Phase 0 exit | Product + safety owners | Scope and hazard-boundary review |
...
| Canonical schema, encoding, commitment fields and snapshot freshness | Open | Phase 1 exit / Phase 2 entry | Architecture + cryptography + security | Reviewed specification and golden vectors |
| Trust-state claim eligibility | Open | Phase 1 exit | Security lead | Threat analysis and signed/unsigned/invalid tests |
| Proof system, circuit toolchain, setup and key lifecycle | Open | Phase 2 exit | Cryptography lead | Benchmark/compatibility ADR and independent review |
...
| Rust/Tokio and MAVLink library | Proposed candidate | Phase 0/1 | Engineering + telemetry leads | Maintained-library review, signing compatibility, fuzz/benchmark result |
...
| Public chain metadata allowlist | Open | Phase 4 entry | Product/privacy + security | Observer-focused privacy review |
...
| Live-chain production, HA, Kubernetes, brokers, cache, object storage, multi-region | Deferred | Separate post-MVP decision | Relevant owner | Demonstrated requirement and operational evidence |
